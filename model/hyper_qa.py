...
class HyperQA(object):
    def __init__(self) -> None:
        ...
        # layers
        self.embedding = tf.keras.layers.Embedding(self.sequence_length, self.embedding_dim)
        ...
        self.loss = tf.reduce_sum(tf.clip_by_value(self.losses, clip_value_min=0))

    def _to_representation(self, sequence):
        embed = self.embedding(sequence)
        projected = self.projection(embed)
        rep = tf.reduce_sum(projected, 1, keep_dims=True)
        return tf.clip_by_norm(rep, 1.0, axis=1)

    def _hyperbolic_distance(self, q, a, eps=1e-16):
        def _square_norm(x):
            return tf.square(tf.norm(x, keep_dims=True, axis=1))
